...
class Inactive_business extends CI_Controller
{
    ...
    public function inactive_business()
    {
        try
        {
            $business = $this->model1->select('business_members', '*', array('status' => 'W'));
            if (true == $business)
            {
            ...
            $this->header = 200;
            $this->message = $business == false ? 'No inactive businesses.' : count($business).' businesses are inactive.';
            $this->root = 'admin/main';
            $this->response();
        } catch (Exception $e)
        {
