...
class Canceled extends CI_Controller
{
    ...
    public function canceled()
    {
        try
        {
            $tra = $this->model1->select('transaction_info', '*', array('payment_status' => 'C'), array('pay_time' => 'asc'));
            if (true == $tra)
            {
                $items = array();
                foreach ($tra as $tr)
            ...
            $this->data['title'] = $this->title('Payments ', 'Cancelled');
            
            $this->page = 'admin/control_payment/control_payment';
            
            $this->header = 200;
            
            $this->message = 'Cancelled payments';
