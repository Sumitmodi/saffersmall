...
class Ads extends CI_Controller {
    ...
    public function enter() {
        try {
            ...
            //echo $this->session->userdata('paymethod'); echo 'hi'; return;

            if (isset($_SESSION['anti_session']))
                $anti_session = $_SESSION['anti_session'];
            ...
            if ($edit_extra == 1 && $anti_session == 0) {

                $this->edit_extra($ad_id);
            }

            $hasAdPlaced = $this->session->userdata('product1');

            if ($hasAdPlaced != false && $anti_session == 0) {
    ...
    private function saveAd() {
        ...
        $product['name'] = $this->session->userdata('product1');
        ...
        $pkg = $this->model->select('app_packages', 'sno,length,price', array('LOWER(name)' => $this->session->userdata('package_name')), NULL, 1);
        ...
        if ($this->session->userdata('ad_type') == 'link_ad')
            $product['is_link_ad'] = 1;
...
//return;
        ...
        if ($added) {
            ...
            $this->session->unset_userdata('product1');
            ...
            // $this->session->set_userdata('anti_session', 1);
//            echo '<pre>'; print_r($this->session->all_userdata());
//            echo 'test';echo $this->session->userdata('product1'); echo 'here';



            if ($this->session->userdata('package_name') != false) {
                ...
                $package['amount_paid'] = $pkg->price * $pkg->length;
...
//                else echo 'no';
                ...
                $this->session->unset_userdata('package_price');
            }
            ...
            if ($this->session->userdata('bump') != false ||
                ...
                if ($this->session->userdata('payment') == 'success') {
                    if (isset($lastId)) {
                        ...
                        $transection = $this->model->insert('transaction_info', $insert);

//                        if($transection) echo 'yes1';
//                        else echo 'no1';
                    }
                }
            }
    ...
    private function modify($id) {
        ...
        $this->data['ad_type'] = $ad->is_link_ad;
        ...
        if ($step == 4) {
            if (!empty($data)) {
                ...
                $extra = $this->model->select('ad_extra', '*', array('ad_id' => $id), null, 1);
                //echo 'hi'; echo '<pre>'; print_r($extra); return;
                $this->data['extra'] = $extra;
...
//echo '<pre>'; print_r($packages);  return;
    ...
    private function edit_extra($id) {
        ...
        $payment_method = $this->session->userdata('paymethod'); 
        ...
        if ($this->session->userdata('bump') != false ||
            ...
            if ($this->session->userdata('payment') == 'success') {
                ...
                if (isset($id)) {
                    ...
                    $insert = array(
                        'payment_method' => $payment_method == false ? 'paypal' : $payment_method,
                        'bid' => $user->sno,
                        'gross_amount' => $this->session->userdata('pay') == false ? 0 : $this->session->userdata('pay'),
                        'received_amount' => $this->session->userdata('pay') == false ? 0 : $this->session->userdata('pay'),
                        'payment_status' => 'R',
