        ...
        try {
            ...
            if (false == $user) {
                ...
                if (false == $q && false == $c) {
                    ...
                    $result = $this->model->select('business_ads', '*', array('LOWER(status)' => 'a', 'is_link_ad' => 0), array('date_added' => 'desc'));
                }
                ...
                if ($q != false && $c != false) {
                    ...
                    if ($q == false) {
                        ...
                        if (false == $cat) {
                            ...
                            $result = $this->model->select('business_ads', '*', array('cat_id' => $cat->sno, 'LOWER(status)' => 'a', 'is_link_ad' => 0), array('date_added' => "desc"));
                        }
                    }
    ...
    private function related() {
        ...
        if ($highlighed != false) {
            ...
            if (count($active) > 0) {
                ...
                if (shuffle($active)) {
                    ...
                    $this->data['sidetop'] = array_rand($active, min(count($active), 4));
                }
            }
        }
        ...
        if ($urgent != false) {
            ...
            foreach ($urgent as $k => $h) {
                ...
                if (!$this->expired($h->end_date)) {
                    $ad = $this->model->select('business_ads', '*', array('sno' => $h->ad_id, 'is_link_ad' => 0), NULL, 1);
            ...
            if (count($active) > 0) {
                ...
                if (shuffle($active)) {
                     ...
                     * We only want to display 4 highlighted ads on the sidebar.
                     */
                    //echo 'hi'; echo'<pre>'; print_r($active);
                    $this->data['sidebot'] = array_rand($active, min(count($active), 4));
                }
            }
        }
    }
