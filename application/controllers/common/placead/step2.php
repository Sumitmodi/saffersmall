...
class Step2 extends CI_Controller {
    ...
    public function manage() {
        try {
            ...
            $userId = $this->model->select('business_members', 'sno', array('username'=>$username), null, 1);
           
            $product = $this->model->select('business_ads', 'name', array('bid'=>$userId->sno, 'name'=>  strtolower($data['name'])), null, 1);
            ...
            if (empty($data['name'])) {
             ...
             if (strpos($data['name'], '-') == TRUE) {
                throw new Exception('Name of the product must not contain "-".', '900');
            }
            
            if(!empty($product)){
                throw new Exception('Name of the product already exist.', '900');
            }
            ...
            //echo $data['type']; echo 'here';
            $this->session->set_userdata('ad_type', $data['ad_type']);

            $this->data['title'] = $this->title('Place AD', 'Step 2');

            $this->session->set_userdata('product1', $data['name']);
