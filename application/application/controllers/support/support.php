...
class Support extends CI_Controller
{
    ...
    public function show($target = NULL)
    {
        try
        {
            ...
            {
                /* show post */
                $post = $this->model1->select('admin_post', '*', array('title' => $target, 'status' => 'A'), NULL, 1);
                $this->data['result'] = $post;

                /* show list of sfi members with its links at the right side of support page */
                $sfi = $this->model1->select('business_members', '*', array('is_sfi' => 1, 'status' => 'A'), NULL, 5);
                ...
                {
                    $cat_ads = $this->model1->select('business_ads', 'sno, name', array('cat_id' => $categories['sno'], 'status !=' => 'W', 'status !=' => 'D'), 'rand()', rand(5, 10));
                    $cat_info[] = array(
                        'category' => $categories,
                        'cat_ads' => $cat_ads
                    );
